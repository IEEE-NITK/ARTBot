...
class ArtistBot(Node):
    ...
    def find_nearest_available_pair(self, bot_name):
        ...
        assigned_pairs = set(pair for pairs in self.pair.values() for pair in pairs)
        ...
        for i in range(len(available_bots)):
            ...
            for j in range(i + 1, len(available_bots)):
                ...
                if pair not in assigned_pairs and bot1 not in self.pair.values() and bot2 not in self.pair.values():
...
def main(args=None):
    ...
    for bot_name, target_coords in target_coordinates_list.items():
